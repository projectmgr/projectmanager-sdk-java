...
public class RestRequest<@NotNull T> {
     ...
     */
    public @NotNull AstroResult<T> Call(TypeToken<AstroResult<T>> token) {
        ...
        try {
            ...
            request.addHeader("SdkVersion", "96.0.2118.0");
            ...
            result.Parse(token, content, response.getCode(), serverDuration, roundTripTime);
        } catch (Exception e) {
            result.Parse(token, e.toString(), -1, -1, -1);
        }
        return result;
    }
}